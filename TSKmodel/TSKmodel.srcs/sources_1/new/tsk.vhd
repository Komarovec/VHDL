...
-- Simple rules
--rule_weights(0) <= x1_mf_vals(0); -- min(x1_low, x2_low) | max(x1_low, x2_low), ...
--rule_weights(1) <= x1_mf_vals(1);
--rule_weights(2) <= x1_mf_vals(2);
...
x1_mbfs_generate: for ii in 0 to X1_MBFS_COUNT-1 generate
    trimf_x1: entity work.trimf port map(
        clk => clk,
